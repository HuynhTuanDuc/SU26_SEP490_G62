-- ╔════════════════════════════════════════════════════════════════════════╗
-- ║  CẢNH BÁO: dòng đầu tiên DROP SCHEMA public CASCADE — chạy file này    ║
-- ║  lên một DB đang có dữ liệu sẽ XOÁ SẠCH TOÀN BỘ, không lấy lại được.   ║
-- ║                                                                        ║
-- ║  File này CHỈ dùng để dựng DB MỚI từ đầu.                              ║
-- ║  DB đang chạy mà cần đổi schema → dùng backend/migrations/              ║
-- ║  (đọc backend/migrations/README.md).                                   ║
-- ╚════════════════════════════════════════════════════════════════════════╝
DROP SCHEMA IF EXISTS public CASCADE;
CREATE SCHEMA public;
CREATE EXTENSION IF NOT EXISTS pgcrypto;

CREATE TABLE roles (
    id   INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    name TEXT UNIQUE NOT NULL
);

INSERT INTO roles (name) VALUES
    ('manager'), ('coordinator'), ('accountant'), ('driver')
ON CONFLICT (name) DO NOTHING;

-- Toàn hệ thống chạy giờ Việt Nam (+07). Postgres mặc định UTC, mà NOW(),
-- CURRENT_DATE, ::date và EXTRACT(MONTH FROM timestamptz) đều quy đổi theo múi giờ
-- phiên — để UTC thì từ 0h đến 7h sáng giờ VN hệ thống vẫn coi là "hôm qua", làm
-- sai mốc tháng của KPI/lương và sai khớp ngày lễ với chuyến chạy đêm.
-- TIMESTAMPTZ lưu UTC bên trong nên đổi múi giờ KHÔNG đụng tới dữ liệu đã có.
DO $$
BEGIN
    EXECUTE format('ALTER DATABASE %I SET timezone = %L', current_database(), 'Asia/Ho_Chi_Minh');
END $$;
SET timezone = 'Asia/Ho_Chi_Minh';

-- Ghi lại các migration đã áp lên DB này. Không có bảng này thì không cách nào
-- biết một DB đang ở schema nào → không dám chạy lại migration, cũng không dám
-- bỏ qua. Mỗi file trong backend/migrations/ tự ghi tên mình vào đây.
CREATE TABLE schema_migrations (
    filename   TEXT PRIMARY KEY,
    applied_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

-- DB dựng mới từ file này đã có sẵn mọi thay đổi schema, nên đánh dấu các
-- migration là "đã áp" để không ai chạy lại lên DB mới.
INSERT INTO schema_migrations (filename) VALUES
    ('20260730_return_flow.sql'),
    ('20260731_holiday_attendance.sql'),
    ('20260731_timezone_vn.sql'),
    ('20260731_expense_idempotency.sql'),
    ('20260801_driver_group_history.sql'),
    ('20260802_import_fingerprint.sql'),
    ('20260802_holiday_bonus_rule.sql'),
    ('20260802_optional_staff_email.sql'),
    ('20260802_customer_name_lookup.sql'),
    ('20260802_manual_debt.sql'),
    ('20260809_collect_on_behalf.sql'),
    ('20260905_collect_on_behalf_ledger.sql'),
    ('20260906_collect_on_behalf_return.sql'),
    ('20260809_driver_expense_reimbursement.sql'),
    ('20260810_push_tickets.sql'),
    ('20260814_expense_accrual_pass_through_split.sql'),
    ('20260830_receipt_ai_validation.sql'),
    ('20260831_receipt_duplicate_keys.sql'),
    ('20260901_drop_chatbot_views.sql'),
    ('20260902_reversal_requests.sql'),
    ('20260903_company_legal_identity.sql'),
    ('20260904_reject_nan_money.sql'),
    ('20260907_receipt_ocr_pipeline.sql')
ON CONFLICT (filename) DO NOTHING;

CREATE TABLE accounts (
    id              INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    -- Email KHÔNG bắt buộc: phần lớn tài xế không có email, bắt buộc chỉ khiến người
    -- nhập bịa địa chỉ giả cho qua form. Định danh đăng nhập có thể là email HOẶC số
    -- điện thoại (profiles.phone) — xem backend/utils/loginIdentifier.js.
    -- UNIQUE vẫn chặn trùng email thật; Postgres coi mỗi NULL là khác nhau nên nhiều
    -- tài khoản cùng để trống không hề đụng nhau. CHECK bên dưới cấm chuỗi rỗng để
    -- "không có email" chỉ có đúng một cách biểu diễn là NULL.
    email           TEXT UNIQUE,
    password_hash   TEXT NOT NULL,
    role_id         INT NOT NULL REFERENCES roles(id),
    is_active       BOOLEAN NOT NULL DEFAULT TRUE,
    must_change_password BOOLEAN NOT NULL DEFAULT FALSE,
    last_login_at   TIMESTAMPTZ,
    created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT chk_accounts_email_not_blank CHECK (email IS NULL OR btrim(email) <> '')
);

CREATE TABLE auth_refresh_tokens (
    token_id                TEXT PRIMARY KEY,
    user_id                 INT NOT NULL REFERENCES accounts(id) ON DELETE CASCADE,
    token_hash              TEXT NOT NULL,
    expires_at              TIMESTAMPTZ NOT NULL,
    revoked_at              TIMESTAMPTZ,
    replaced_by_token_id    TEXT,
    created_at              TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE TABLE profiles (
    id                  INT PRIMARY KEY REFERENCES accounts(id) ON DELETE CASCADE,
    full_name           TEXT NOT NULL,
    phone               TEXT UNIQUE,
    role_id             INT NOT NULL REFERENCES roles(id),
    avatar_url          TEXT,
    dob                 DATE,
    gender              TEXT CHECK (gender IN ('male','female','other')),
    national_id         TEXT,
    tax_code            TEXT,
    address             TEXT,
    city                TEXT,
    country             TEXT NOT NULL DEFAULT 'VN',
    emergency_contact_name  TEXT,
    emergency_contact_phone TEXT,
    notes               TEXT,
    created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at          TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE TABLE vehicle_groups (
    id                  INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    name                TEXT NOT NULL UNIQUE,
    description         TEXT,
    max_load_weight_kg  NUMERIC(10,2),
    price_per_km        NUMERIC(12,2) NOT NULL,
    status              TEXT NOT NULL DEFAULT 'active'
                            CHECK (status IN ('active','hidden')),
    upgrade_allowed     BOOLEAN NOT NULL DEFAULT FALSE,
    created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE TABLE vehicles (
    id                  INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    plate_number        TEXT UNIQUE NOT NULL,
    vehicle_group_id    INT NOT NULL REFERENCES vehicle_groups(id),
    brand               TEXT,
    model               TEXT,
    load_capacity_kg    NUMERIC(10,2),
    manufacture_year    SMALLINT,
    purchase_date       DATE,
    assigned_driver_id  INT REFERENCES profiles(id) UNIQUE,
    status              TEXT NOT NULL DEFAULT 'active'
                            CHECK (status IN ('active','retired','maintenance','broken')),
    created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at          TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE TABLE drivers (
    profile_id              INT PRIMARY KEY REFERENCES profiles(id) ON DELETE CASCADE,
    vehicle_id              INT UNIQUE REFERENCES vehicles(id),
    default_vehicle_group_id INT REFERENCES vehicle_groups(id),
    license_number          TEXT NOT NULL,
    license_expiry_date     DATE,
    hire_date               DATE NOT NULL,
    emergency_contact_name  TEXT,
    emergency_contact_phone TEXT,
    revenue_share_percent   NUMERIC(5,2) NOT NULL DEFAULT 15.00,
    created_at              TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE TABLE customers (
    id              INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    customer_type   TEXT NOT NULL CHECK (customer_type IN ('individual','business')),
    full_name       TEXT,
    company_name    TEXT,
    contact_person  TEXT,
    phone           TEXT NOT NULL,
    email           TEXT,
    address         TEXT,
    tax_code        TEXT,
    notes           TEXT,
    created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at      TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

-- Tra khách theo tên đã chuẩn hoá (dòng import chỉ có tên, không có SĐT).
-- Biểu thức phải trùng khít tenKhachChuanSql() trong accountantOrderRepository.js.
CREATE INDEX idx_customers_ten_chuan
    ON customers (lower(regexp_replace(btrim(COALESCE(full_name, company_name, '')), '\s+', ' ', 'g')));

CREATE TABLE partners (
    id                          INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    company_name                TEXT NOT NULL,
    short_name                  TEXT,
    contact_person              TEXT,
    phone                       TEXT,
    email                       TEXT,
    address                     TEXT,
    tax_code                    TEXT,
    business_registration_number TEXT,
    payment_term_days           SMALLINT,
    bank_name                   TEXT,
    bank_account_number         TEXT,
    bank_account_name           TEXT,
    notes                       TEXT,
    created_at                  TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE TABLE orders (
    id              INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    customer_id     INT REFERENCES customers(id),
    created_by      INT NOT NULL REFERENCES profiles(id),
    updated_by      INT REFERENCES profiles(id),
    partner_name TEXT,
    partner_id          INT REFERENCES partners(id),
    cargo_name          TEXT,
    cargo_weight_kg     NUMERIC(10,2),
    payment_type        TEXT CHECK (payment_type IN ('cash','bank_transfer','client_credit')),

    total_estimated_price   NUMERIC(12,2) NOT NULL DEFAULT 0,
    prepaid_amount         NUMERIC(12,2) NOT NULL DEFAULT 0 CHECK (prepaid_amount >= 0),
    prepaid_status         TEXT NOT NULL DEFAULT 'none' CHECK (prepaid_status IN ('none','pending','confirmed')),
    prepaid_method         TEXT CHECK (prepaid_method IN ('cash','bank_transfer')),
    prepaid_proof_url      TEXT,
    prepaid_confirmed_by   INT REFERENCES profiles(id),
    prepaid_confirmed_at   TIMESTAMPTZ,
    derived_status      TEXT NOT NULL DEFAULT 'open'
                            CHECK (derived_status IN ('open','completed','cancelled','partial')),
    is_confidential     BOOLEAN NOT NULL DEFAULT FALSE,
    notes               TEXT,
    import_batch_id     TEXT,
    -- Vân tay dòng Excel đã import (SHA-256) — chặn import lại cùng một dòng.
    -- NULL = nhập tay, hoặc kế toán chủ động cho phép trùng.
    import_fingerprint  TEXT,
    created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at          TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

-- UNIQUE có điều kiện: chỉ ràng buộc các đơn đến từ import, đơn nhập tay không đụng tới.
CREATE UNIQUE INDEX uq_orders_import_fingerprint
    ON orders (import_fingerprint)
    WHERE import_fingerprint IS NOT NULL;

CREATE TABLE order_shipments (
    id                    INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    order_id              INT NOT NULL REFERENCES orders(id) ON DELETE CASCADE,
    shipment_index        SMALLINT NOT NULL,
    vehicle_group_id      INT REFERENCES vehicle_groups(id),
    estimated_price       NUMERIC(12,2),
    is_price_manual       BOOLEAN NOT NULL DEFAULT FALSE,
    estimated_distance_km NUMERIC(10,2),
    actual_distance_km    NUMERIC(10,2),
    actual_price          NUMERIC(12,2),
    -- Thu hộ (COD): tiền hàng công ty thu hộ khách khi giao chuyến này.
    -- KHÔNG phải doanh thu, KHÔNG cấn vào công nợ cước — là tiền của khách mà công ty
    -- đang giữ (chiều ngược với công nợ), theo dõi song song với debts.total_amount.
    -- Đặt ở đây chứ không ở debts vì chuyến đã thu đủ cước thì không có dòng nợ nào để đựng.
    collect_on_behalf_amount NUMERIC(12,2) NOT NULL DEFAULT 0
                              CHECK (collect_on_behalf_amount >= 0),
    cargo_name            TEXT,
    cargo_weight_kg       NUMERIC(10,2),
    status                TEXT NOT NULL DEFAULT 'available'
                              CHECK (status IN (
                                  'available',
                                  'claimed',
                                  'picking',
                                  'transit',
                                  'arrived',
                                  'completed',
                                  'failed',
                                  'returning',
                                  'cancelled'
                              )),

    version               INT NOT NULL DEFAULT 0,

    notes                 TEXT,
    cancel_reason         TEXT,

    -- Giao thất bại: điều phối chọn cho giao lại hay cho hoàn hàng (ở màn Sự cố).
    -- Hoàn hàng luôn tính GẤP ĐÔI cước nên không có tùy chọn tiền nào để lưu.
    failed_resolved_by    INT REFERENCES profiles(id),
    failed_resolved_at    TIMESTAMPTZ,

    claimed_at            TIMESTAMPTZ,
    picking_at            TIMESTAMPTZ,
    transit_at            TIMESTAMPTZ,
    arrived_at            TIMESTAMPTZ,
    completed_at          TIMESTAMPTZ,
    failed_at             TIMESTAMPTZ,
    returning_at          TIMESTAMPTZ,
    cancelled_at          TIMESTAMPTZ,

    -- Tháng GHI NHẬN doanh thu (ngày 1 của tháng), tách khỏi ngày chạy: chuyến chạy
    -- tháng 8 mà phiếu thu duyệt sau khi tháng 8 đã ký duyệt thì doanh thu được đẩy
    -- sang kỳ mở sớm nhất, thay vì rơi mất khỏi mọi báo cáo. Trigger set_revenue_period
    -- ở cuối file tự điền. NULL = theo tháng của completed_at.
    revenue_period        DATE,

    created_at            TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at            TIMESTAMPTZ NOT NULL DEFAULT NOW(),

    UNIQUE (order_id, shipment_index)
);

CREATE TABLE trip_stops (
    id              INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    shipment_id     INT NOT NULL REFERENCES order_shipments(id) ON DELETE CASCADE,
    stop_index      SMALLINT NOT NULL,
    stop_type       TEXT NOT NULL CHECK (stop_type IN ('pickup','delivery')),
    address         TEXT NOT NULL,
    contact_name    TEXT,
    contact_phone   TEXT,
    notes           TEXT,
    arrived_at      TIMESTAMPTZ,
    completed_at    TIMESTAMPTZ,
    proof_url       TEXT,
    created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),

    UNIQUE (shipment_id, stop_index)
);

CREATE TABLE order_receipt_requests (
    id                      INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    order_id                INT NOT NULL REFERENCES orders(id) ON DELETE CASCADE,

    requesting_shipment_id  INT NOT NULL REFERENCES order_shipments(id),
    driver_id               INT NOT NULL REFERENCES profiles(id),

    driver_notes            TEXT,

    status                  TEXT NOT NULL DEFAULT 'pending'
                                CHECK (status IN ('pending','processing','approved','rejected')),

    requested_at            TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    processed_by            INT REFERENCES profiles(id),
    processed_at            TIMESTAMPTZ,
    coordinator_notes       TEXT,

    CONSTRAINT uq_order_receipt_request UNIQUE (order_id)
);

CREATE TABLE shipment_receipts (
    id                  INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    shipment_id         INT NOT NULL REFERENCES order_shipments(id) ON DELETE CASCADE,
    payment_type        TEXT CHECK (payment_type IN (
                            'cash_collected', 'bank_transfer', 'client_credit'
                        )),
    amount              NUMERIC(12,2) NOT NULL CHECK (amount >= 0),
    collected_by        INT REFERENCES profiles(id),
    collected_at        TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    notes               TEXT,
    order_receipt_request_id  INT REFERENCES order_receipt_requests(id),
    created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    created_by          INT REFERENCES profiles(id)
);

CREATE TABLE payment_receipts (
    id          INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    payment_id  INT NOT NULL REFERENCES shipment_receipts(id) ON DELETE CASCADE,
    file_url    TEXT NOT NULL,
    uploaded_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE TABLE company_info (
    id                  INT PRIMARY KEY DEFAULT 1,
    company_name        TEXT,
    -- Mã số thuế + địa chỉ trụ sở: khối định danh đơn vị ở góc trên trái mọi biểu mẫu
    -- kế toán xuất ra. Thiếu thì sổ in ra không dùng để nộp hay lưu trữ được.
    tax_code            TEXT,
    address             TEXT,
    hotline             TEXT,
    bank_name           TEXT,
    bank_account_number TEXT,
    bank_account_name   TEXT,
    bank_qr_url         TEXT,
    -- Tran khau tru cong no tai xe moi ky luong, tinh theo % so con duoc nhan sau khi
    -- tru BHXH/ung luong/nghi khong luong. Khong co tran thi mot khoan no cu lon se an
    -- sach luong thang do va net_salary ra so am.
    driver_debt_monthly_cap_percent NUMERIC(5,2) NOT NULL DEFAULT 30
                        CHECK (driver_debt_monthly_cap_percent > 0 AND driver_debt_monthly_cap_percent <= 100),
    updated_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_by          INT REFERENCES profiles(id)
);
INSERT INTO company_info (id) VALUES (1) ON CONFLICT DO NOTHING;

CREATE TABLE delivery_proofs (
    id              INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    shipment_id     INT NOT NULL REFERENCES order_shipments(id) ON DELETE CASCADE,
    captured_by     INT NOT NULL REFERENCES profiles(id),
    file_url        TEXT NOT NULL,
    is_realtime     BOOLEAN NOT NULL DEFAULT TRUE,
    captured_at     TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE TABLE expenses (
    id              INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    shipment_id     INT REFERENCES order_shipments(id),
    vehicle_id      INT REFERENCES vehicles(id),
    created_by      INT NOT NULL REFERENCES profiles(id),
    updated_by      INT REFERENCES profiles(id),
    expense_type    TEXT NOT NULL
                        CHECK (expense_type IN (
                            'toll','parking','etc',
                            'fuel','repair',
                            'maintenance','depreciation','other'
                        )),
    amount          NUMERIC(12,2) NOT NULL CHECK (amount > 0 AND amount < 'Infinity'::numeric),
    description     TEXT,
    expense_date    DATE NOT NULL DEFAULT CURRENT_DATE,

    -- Khoá chống trùng do CLIENT sinh. App tài xế lưu chi phí vào hàng đợi khi mất
    -- mạng rồi tự gửi lại lúc có sóng; không có khoá này thì một lần gửi lại là
    -- thành hai bản ghi chi phí. Các luồng khác được máy trạng thái chặn sẵn, riêng
    -- khai chi phí thì không có gì chặn nên phải chặn ở đây.
    client_request_id TEXT UNIQUE,
    status          TEXT NOT NULL DEFAULT 'approved'
                        CHECK (status IN ('pending','approved','rejected')),
    reviewed_by     INT REFERENCES profiles(id),
    reviewed_at     TIMESTAMPTZ,
    reject_reason   TEXT,
    reimbursement_status TEXT
                        CHECK (reimbursement_status IN ('pending','offset_debt','paid_via_payroll','settled')),
    reimbursed_at   TIMESTAMPTZ,
    created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at      TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE TABLE expense_attachments (
    id          INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    expense_id  INT NOT NULL REFERENCES expenses(id) ON DELETE CASCADE,
    file_url    TEXT NOT NULL,
    uploaded_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE TABLE maintenance_records (
    id                  INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    vehicle_id          INT NOT NULL REFERENCES vehicles(id),
    expense_id          INT REFERENCES expenses(id),
    maintenance_type    TEXT NOT NULL
                            CHECK (maintenance_type IN ('scheduled','repair','inspection','emergency')),
    description         TEXT,
    -- < 'Infinity' loại được cả NaN: Postgres xếp NaN lớn hơn mọi số, và một dòng
    -- NaN làm mọi SUM() chạm tới nó trả về NaN — xem migration 20260904.
    cost                NUMERIC(12,2) CHECK (cost IS NULL OR cost < 'Infinity'::numeric),
    maintenance_date    DATE NOT NULL,
    next_due_date       DATE,
    performed_by        INT NOT NULL REFERENCES drivers(profile_id),
    status              TEXT NOT NULL DEFAULT 'open'
                            CHECK (status IN ('requested','open','pending_verification','completed','rejected')),
    requested_by        INT REFERENCES profiles(id),
    request_reason      TEXT,
    reject_reason       TEXT,
    started_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    completed_at        TIMESTAMPTZ,
    bill_pics           JSONB NOT NULL DEFAULT '[]'::jsonb,
    created_by          INT REFERENCES profiles(id),
    completed_by        INT REFERENCES profiles(id),
    verified_by         INT REFERENCES profiles(id),
    verified_at         TIMESTAMPTZ,
    created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at          TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

-- Phần MỞ RỘNG của từ điển hạng mục bảo dưỡng. Danh sách gốc nằm trong code
-- (backend/services/receiptTaxonomy.js) — bảng này để nuôi thêm từ khoá lúc chạy mà
-- không cần deploy, chủ yếu từ những lần người duyệt sửa lại phân loại của máy.
CREATE TABLE maintenance_item_keywords (
    id          INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    -- Đã chuẩn hoá: bỏ dấu, chữ thường, gộp khoảng trắng (receiptTaxonomy.normalize).
    keyword     TEXT NOT NULL UNIQUE,
    category    TEXT NOT NULL,
    item_group  TEXT NOT NULL CHECK (item_group IN ('maintenance', 'excluded')),
    source      TEXT NOT NULL DEFAULT 'manual' CHECK (source IN ('manual', 'learned')),
    created_by  INT REFERENCES profiles(id),
    created_at  TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

-- Lưu vết mỗi lần đọc hóa đơn bằng AI: nguyên văn model đọc được + kết luận của từng
-- lớp kiểm tra. Tách hai cột là có chủ đích — raw_extraction là "máy đọc được gì",
-- checks là "hệ thống kết luận gì". Cần cho tranh chấp, cho việc đo độ chính xác thật,
-- và cho vòng phản hồi khi người duyệt ghi đè phán quyết của máy.
CREATE TABLE receipt_extractions (
    id              INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    entity_type     TEXT NOT NULL CHECK (entity_type IN ('maintenance_record', 'expense')),
    entity_id       INT,
    image_url       TEXT NOT NULL,
    -- Băm nội dung ảnh, dùng để chặn nộp lại đúng một tấm ảnh.
    image_sha256    TEXT,
    provider        TEXT,
    model           TEXT,
    prompt_version  TEXT,
    raw_extraction  JSONB,
    checks          JSONB NOT NULL DEFAULT '[]'::jsonb,
    verdict         TEXT NOT NULL
                        CHECK (verdict IN ('passed', 'needs_review', 'rejected', 'error')),
    claimed_amount  NUMERIC(12,2),
    receipt_total   NUMERIC(12,2),
    latency_ms      INT,
    -- Khoá định danh NỘI DUNG tờ hóa đơn, để chặn việc dùng lại nó cho khoản khác.
    -- Băm ảnh chỉ bắt được đúng một file gửi lại; chụp lại cùng tờ giấy từ góc khác là
    -- băm khác. Cặp (bên bán, số hóa đơn) mới là thứ định danh tờ giấy.
    vendor_key      TEXT,
    invoice_no_key  TEXT,
    -- Vết của kênh đọc THỨ HAI (Tesseract), chạy song song với model. Model có thể bịa
    -- ra một con số hợp lý đến từng chữ số; OCR thì chỉ báo cáo được hình dạng ký tự
    -- thật sự có trên ảnh. Giữ nguyên văn text quét được để khi tranh chấp còn một
    -- bằng chứng độc lập đọc được bằng mắt, không phải gọi lại API nào.
    ocr_text        TEXT,
    ocr_confidence  NUMERIC(5,2),
    ocr_engine      TEXT,
    -- Điểm tin cậy 0..1 chấm bằng cách đối chiếu chéo hai kênh đọc
    -- (backend/services/receiptCrossCheck.js). Tách thành cột riêng vì đây là thứ cần
    -- lọc và xếp hạng: "cho tôi xem hóa đơn máy đọc không chắc" là truy vấn chính của
    -- người duyệt. NULL = bản ghi có trước khi có lớp đối chiếu chéo.
    confidence      NUMERIC(4,3),
    image_width     INT,
    image_height    INT,
    -- Tóm tắt từng giai đoạn của dây chuyền đã làm gì và trừ điểm ở đâu. JSONB vì hình
    -- dạng còn đổi theo từng lần cải tiến; phần cần truy vấn ổn định đã kéo ra cột riêng.
    pipeline        JSONB,
    reviewed_by     INT REFERENCES profiles(id),
    review_action   TEXT CHECK (review_action IN ('agree', 'override_accept', 'override_reject')),
    review_note     TEXT,
    reviewed_at     TIMESTAMPTZ,
    created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE INDEX idx_receipt_extractions_entity  ON receipt_extractions (entity_type, entity_id);
CREATE INDEX idx_receipt_extractions_hash    ON receipt_extractions (image_sha256) WHERE image_sha256 IS NOT NULL;
CREATE INDEX idx_receipt_extractions_verdict ON receipt_extractions (verdict, created_at DESC);
-- Chỉ đánh index phần có đủ khoá: hóa đơn không ghi số thì cặp này không định danh gì.
CREATE INDEX idx_receipt_extractions_invoice_identity ON receipt_extractions (vendor_key, invoice_no_key)
    WHERE vendor_key IS NOT NULL AND invoice_no_key IS NOT NULL;
-- Chỉ phần đã có điểm: bản ghi cũ (confidence NULL) không bao giờ là kết quả của truy
-- vấn "hóa đơn máy đọc không chắc".
CREATE INDEX idx_receipt_extractions_confidence ON receipt_extractions (confidence, created_at DESC)
    WHERE confidence IS NOT NULL;

CREATE TABLE vehicle_status_history (
    id                  INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    vehicle_id          INT NOT NULL REFERENCES vehicles(id),
    action_type         TEXT NOT NULL
                            CHECK (action_type IN (
                                'send_to_maintenance',
                                'complete_maintenance',
                                'mark_broken',
                                'restore_vehicle',
                                'retire_vehicle'
                            )),
    from_status         TEXT NOT NULL
                            CHECK (from_status IN ('active','maintenance','broken','retired')),
    to_status           TEXT NOT NULL
                            CHECK (to_status IN ('active','maintenance','broken','retired')),
    reference_type      TEXT,
    reference_id        INT,
    note                TEXT,
    metadata            JSONB,
    created_by          INT REFERENCES profiles(id),
    created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE TABLE vehicle_driver_assignments (
    id                  INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    vehicle_id          INT NOT NULL REFERENCES vehicles(id),
    driver_id           INT REFERENCES profiles(id),
    previous_driver_id  INT REFERENCES profiles(id),
    action              TEXT NOT NULL CHECK (action IN ('assign','unassign')),
    note                TEXT,
    created_by          INT REFERENCES profiles(id),
    created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE TABLE debts (
    id              INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    debt_type       TEXT NOT NULL CHECK (debt_type IN ('customer','partner','driver')),
    customer_id     INT REFERENCES customers(id),
    partner_id      INT REFERENCES partners(id),
    driver_id       INT REFERENCES profiles(id),
    order_id        INT REFERENCES orders(id),
    shipment_id     INT REFERENCES order_shipments(id),
    total_amount    NUMERIC(12,2) NOT NULL CHECK (total_amount > 0 AND total_amount < 'Infinity'::numeric),
    due_date        DATE,
    notes           TEXT,
    -- shipment = sinh tu chuyen; manual = ke toan khai tay (thuong la no cu truoc khi
    -- dung phan mem). Phan biet de biet khoan nao duoc phep sua/xoa.
    source          TEXT NOT NULL DEFAULT 'shipment' CHECK (source IN ('shipment','manual')),
    -- Ngay khoan no THAT SU phat sinh — khac created_at la ngay khai vao he thong.
    incurred_on     DATE,
    created_by      INT REFERENCES profiles(id),
    updated_by      INT REFERENCES profiles(id),
    created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at      TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE INDEX idx_debts_source ON debts(source) WHERE source = 'manual';

CREATE TABLE debt_payments (
    id              INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    debt_id         INT NOT NULL REFERENCES debts(id) ON DELETE CASCADE,
    amount          NUMERIC(12,2) NOT NULL CHECK (amount > 0 AND amount < 'Infinity'::numeric),
    payment_method  TEXT CHECK (payment_method IN ('cash','bank_transfer','offset')),
    status          TEXT NOT NULL DEFAULT 'pending'
                        CHECK (status IN ('pending','confirmed','rejected','voided')),
    receipt_url     TEXT,
    reject_reason   TEXT,
    paid_at         TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    confirmed_at    TIMESTAMPTZ,
    confirmed_by    INT REFERENCES profiles(id),
    created_by      INT NOT NULL REFERENCES profiles(id),
    notes           TEXT
);

CREATE TABLE invoices (
    id              INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    invoice_number  TEXT UNIQUE NOT NULL,
    customer_id     INT NOT NULL REFERENCES customers(id),
    order_id        INT REFERENCES orders(id),
    created_by      INT NOT NULL REFERENCES profiles(id),
    invoice_date    DATE NOT NULL DEFAULT CURRENT_DATE,
    due_date        DATE,
    tax_rate        NUMERIC(5,2) NOT NULL DEFAULT 10.00,
    subtotal        NUMERIC(12,2) NOT NULL DEFAULT 0,
    tax_amount      NUMERIC(12,2) GENERATED ALWAYS AS (subtotal * tax_rate / 100) STORED,
    total_amount    NUMERIC(12,2) GENERATED ALWAYS AS (subtotal + subtotal * tax_rate / 100) STORED,
    status          TEXT NOT NULL DEFAULT 'draft'
                        CHECK (status IN ('draft','sent','partial','paid','overdue','cancelled')),
    notes           TEXT,
    created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at      TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE TABLE invoice_shipments (
    id              INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    invoice_id      INT NOT NULL REFERENCES invoices(id) ON DELETE CASCADE,
    shipment_id     INT NOT NULL REFERENCES order_shipments(id),
    line_amount     NUMERIC(12,2) NOT NULL,
    UNIQUE (invoice_id, shipment_id)
);

-- Lịch sử đổi nhóm xe cố định của tài xế.
--
-- Đổi nhóm là thao tác ĐỤNG THẲNG VÀO TIỀN: nhóm quyết định ngưỡng thưởng vượt KPI
-- (50/65/70/100 triệu tuỳ nhóm) và quyết định tài tranh giải "lái xe xuất sắc" ở
-- bảng nào. Trước đây chỉ UPDATE drivers rồi thôi — không biết ai đổi, lúc nào, từ
-- nhóm nào sang nhóm nào. Hệ thống đã có lịch sử cho những thứ ít nhạy cảm hơn
-- (vehicle_status_history, shipment_assignment_history) nên chỗ này phải có.
CREATE TABLE driver_vehicle_group_history (
    id                    INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    driver_id             INT NOT NULL REFERENCES profiles(id),
    from_vehicle_group_id INT REFERENCES vehicle_groups(id),   -- NULL khi gán lần đầu
    to_vehicle_group_id   INT NOT NULL REFERENCES vehicle_groups(id),
    changed_by            INT NOT NULL REFERENCES profiles(id),
    reason                TEXT,

    -- Các kỳ KPI đã được cập nhật theo nhóm mới, dạng "7/2026, 8/2026".
    -- Rỗng nghĩa là không kỳ nào (lương đều đã chốt) → nhóm mới chỉ áp dụng từ kỳ sau.
    applied_periods       TEXT,
    kpi_synced            BOOLEAN NOT NULL DEFAULT FALSE,

    created_at            TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE TABLE kpi_records (
    id                      INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    driver_id               INT NOT NULL REFERENCES profiles(id),
    vehicle_group_id        INT NOT NULL REFERENCES vehicle_groups(id),
    month                   SMALLINT NOT NULL CHECK (month BETWEEN 1 AND 12),
    year                    SMALLINT NOT NULL CHECK (year >= 2020),
    completed_shipments     NUMERIC(8,1) NOT NULL DEFAULT 0,
    total_revenue           NUMERIC(12,2) NOT NULL DEFAULT 0,
    incident_count          INT NOT NULL DEFAULT 0,
    major_incident_count    INT NOT NULL DEFAULT 0,
    critical_incident_count INT NOT NULL DEFAULT 0,
    created_at              TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at              TIMESTAMPTZ NOT NULL DEFAULT NOW(),

    UNIQUE (driver_id, month, year)
);

CREATE TABLE bonus_rules (
    id                  INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    vehicle_group_id    INT REFERENCES vehicle_groups(id),
    title               TEXT NOT NULL,
    bonus_type          TEXT NOT NULL
                            CHECK (bonus_type IN (
                                'kpi','top_revenue','top_trips',
                                'zero_incident','overtime','holiday','custom'
                            )),
    reward_amount       NUMERIC(12,2),
    reward_multiplier   NUMERIC(5,2),
    conditions_json     JSONB,
    is_active           BOOLEAN NOT NULL DEFAULT TRUE,
    created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE TABLE bonus_records (
    id              INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    driver_id       INT NOT NULL REFERENCES profiles(id),
    bonus_rule_id   INT REFERENCES bonus_rules(id),
    kpi_record_id   INT REFERENCES kpi_records(id),
    amount          NUMERIC(12,2) NOT NULL CHECK (amount > 0),
    awarded_month   SMALLINT NOT NULL CHECK (awarded_month BETWEEN 1 AND 12),
    awarded_year    SMALLINT NOT NULL,
    notes           TEXT,
    created_by      INT NOT NULL REFERENCES profiles(id),
    created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE TABLE company_holidays (
    holiday_date DATE PRIMARY KEY,
    name         TEXT NOT NULL
);

INSERT INTO company_holidays (holiday_date, name) VALUES
    ('2026-01-01', 'Tết Dương lịch'),
    ('2026-02-17', 'Tết Âm lịch (mùng 1)'),
    ('2026-02-18', 'Tết Âm lịch (mùng 2)'),
    ('2026-02-19', 'Tết Âm lịch (mùng 3)'),
    ('2026-04-26', 'Giỗ Tổ Hùng Vương (10/03 ÂL)'),
    ('2026-04-30', 'Ngày Giải phóng miền Nam'),
    ('2026-05-01', 'Ngày Quốc tế lao động'),
    ('2026-09-02', 'Ngày Quốc khánh');

CREATE TABLE payrolls (
    id                      INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    driver_id               INT NOT NULL REFERENCES profiles(id),
    payroll_month           SMALLINT NOT NULL CHECK (payroll_month BETWEEN 1 AND 12),
    payroll_year            SMALLINT NOT NULL,

    base_salary             NUMERIC(12,2) NOT NULL,
    months_of_service       INT NOT NULL,

    total_revenue           NUMERIC(12,2) NOT NULL DEFAULT 0,
    revenue_share_pct       NUMERIC(5,2) NOT NULL DEFAULT 15.00,
    revenue_bonus           NUMERIC(12,2) NOT NULL DEFAULT 0,

    kpi_bonus               NUMERIC(12,2) NOT NULL DEFAULT 0,
    top_driver_bonus        NUMERIC(12,2) NOT NULL DEFAULT 0,
    overtime_bonus          NUMERIC(12,2) NOT NULL DEFAULT 0,
    holiday_bonus           NUMERIC(12,2) NOT NULL DEFAULT 0,
    other_bonus             NUMERIC(12,2) NOT NULL DEFAULT 0,

    manual_bonus            NUMERIC(12,2) NOT NULL DEFAULT 0 CHECK (manual_bonus >= 0),

    insurance_employee      NUMERIC(12,2) NOT NULL DEFAULT 0,
    insurance_company       NUMERIC(12,2) NOT NULL DEFAULT 0,
    driver_debt_deduction   NUMERIC(12,2) NOT NULL DEFAULT 0,
    advance_deduction       NUMERIC(12,2) NOT NULL DEFAULT 0,
    absence_penalty         NUMERIC(12,2) NOT NULL DEFAULT 0,
    other_deduction         NUMERIC(12,2) NOT NULL DEFAULT 0,
    manual_deduction        NUMERIC(12,2) NOT NULL DEFAULT 0 CHECK (manual_deduction >= 0),

    expense_reimbursement   NUMERIC(12,2) NOT NULL DEFAULT 0,

    gross_salary            NUMERIC(12,2) GENERATED ALWAYS AS (
                                base_salary + revenue_bonus
                                + kpi_bonus + top_driver_bonus
                                + overtime_bonus + holiday_bonus + other_bonus
                                + manual_bonus
                            ) STORED,

    net_salary              NUMERIC(12,2) GENERATED ALWAYS AS (
                                base_salary + revenue_bonus
                                + kpi_bonus + top_driver_bonus
                                + overtime_bonus + holiday_bonus + other_bonus
                                + manual_bonus
                                + expense_reimbursement
                                - insurance_employee
                                - driver_debt_deduction
                                - advance_deduction
                                - absence_penalty
                                - other_deduction
                                - manual_deduction
                            ) STORED,

    status                  TEXT NOT NULL DEFAULT 'pending'
                                CHECK (status IN ('pending','reviewed','approved','paid')),
    reviewed_by             INT REFERENCES profiles(id),
    reviewed_at             TIMESTAMPTZ,
    approved_by             INT REFERENCES profiles(id),
    approved_at             TIMESTAMPTZ,
    paid_by                 INT REFERENCES profiles(id),
    paid_at                 TIMESTAMPTZ,

    adjusted_by             INT REFERENCES profiles(id),
    adjusted_at             TIMESTAMPTZ,
    adjustment_note         TEXT,

    created_at              TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at              TIMESTAMPTZ NOT NULL DEFAULT NOW(),

    UNIQUE (driver_id, payroll_month, payroll_year)
);

CREATE TABLE salary_advances (
    id              INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    driver_id       INT NOT NULL REFERENCES profiles(id),
    amount          NUMERIC(12,2) NOT NULL CHECK (amount > 0 AND amount < 'Infinity'::numeric),
    reason          TEXT,
    request_month   SMALLINT NOT NULL CHECK (request_month BETWEEN 1 AND 12),
    request_year    SMALLINT NOT NULL,
    status          TEXT NOT NULL DEFAULT 'pending'
                        CHECK (status IN ('pending','approved','rejected','paid')),
    reviewed_by     INT REFERENCES profiles(id),
    reviewed_at     TIMESTAMPTZ,
    approved_by     INT REFERENCES profiles(id),
    approved_at     TIMESTAMPTZ,
    paid_by         INT REFERENCES profiles(id),
    paid_at         TIMESTAMPTZ,
    reject_reason   TEXT,
    created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at      TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE TABLE leave_requests (
    id          INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    driver_id   INT NOT NULL REFERENCES profiles(id),
    leave_date  DATE NOT NULL,
    leave_type  TEXT NOT NULL DEFAULT 'unpaid'
                    CHECK (leave_type IN ('paid', 'unpaid')),
    reason      TEXT,
    status      TEXT NOT NULL DEFAULT 'approved'
                    CHECK (status IN ('approved', 'rejected')),
    created_at  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    UNIQUE (driver_id, leave_date)
);

-- holiday_worked: tài xế đi làm vào ngày lễ → ngày đó tính 200% lương (Điều V.1).
-- Chỉ dùng để chấm tay khi tài có đi làm nhưng không có chuyến nào hoàn thành đúng
-- ngày đó (chạy xuyên đêm, trực kho, hỗ trợ tài khác...). Nếu đã có chuyến hoàn thành
-- trong ngày lễ thì hệ thống tự nhận, không cần đánh dấu.
CREATE TABLE attendance_overrides (
    id          INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    driver_id   INT NOT NULL REFERENCES profiles(id),
    work_date   DATE NOT NULL,
    status      TEXT NOT NULL CHECK (status IN ('present', 'absent_unexcused', 'half_day', 'holiday_worked')),
    notes       TEXT,
    marked_by   INT NOT NULL REFERENCES profiles(id),
    created_at  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    UNIQUE (driver_id, work_date)
);

CREATE TABLE incidents (
    id              INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    shipment_id     INT REFERENCES order_shipments(id) ON DELETE CASCADE,
    vehicle_id      INT REFERENCES vehicles(id),
    reported_by     INT NOT NULL REFERENCES profiles(id),
    incident_type   TEXT NOT NULL
                        CHECK (incident_type IN (
                            'vehicle_breakdown',
                            'cargo_damage',
                            'road_incident',
                            'customer_refusal',
                            'traffic_jam',
                            'other'
                        )),
    severity_level  TEXT NOT NULL DEFAULT 'medium'
                        CHECK (severity_level IN ('low','medium','high','critical')),
    description     TEXT NOT NULL,
    location        TEXT,
    estimated_loss  NUMERIC(12,2) DEFAULT 0,
    status          TEXT NOT NULL DEFAULT 'open'
                        CHECK (status IN ('open','investigating','resolved','closed')),

    resolved_by             INT REFERENCES profiles(id),
    resolution_note         TEXT,
    replacement_vehicle_id  INT REFERENCES vehicles(id),
    replacement_driver_id   INT REFERENCES profiles(id),

    compensation_status     TEXT NOT NULL DEFAULT 'none'
                                CHECK (compensation_status IN ('none','pending','approved','rejected','paid','cancelled')),

    occurred_at     TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    resolved_at     TIMESTAMPTZ,
    created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at      TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE TABLE incident_evidences (
    id          INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    incident_id INT NOT NULL REFERENCES incidents(id) ON DELETE CASCADE,
    file_url    TEXT NOT NULL,
    uploaded_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE TABLE notifications (
    id          INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    user_id     INT NOT NULL REFERENCES profiles(id) ON DELETE CASCADE,
    title       TEXT NOT NULL,
    body        TEXT,
    type        TEXT,
    entity_type TEXT,
    entity_id   INT,
    is_read     BOOLEAN NOT NULL DEFAULT FALSE,
    created_at  TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE TABLE activity_logs (
    id          BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    user_id     INT REFERENCES profiles(id),
    action      TEXT NOT NULL,
    entity_type TEXT NOT NULL,
    entity_id   INT,
    old_data    JSONB,
    new_data    JSONB,
    ip_address  INET,
    created_at  TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

-- Yêu cầu hoàn tác tầng 2: người làm sai xin lùi, người có quyền duyệt.
--
-- Các thao tác tầng 2 (gỡ duyệt chi phí, huỷ xác nhận khoản nộp, gỡ hoàn thành chuyến)
-- đều đụng tới tiền. Để ai có quyền là tự làm tự lùi thì chính người bấm nhầm sẽ tự gỡ
-- và không ai biết — đó là chỗ dễ bị lạm dụng nhất.
--
-- Bảng này KHÔNG tự thực hiện việc lùi. Nó ghi lại "ai xin lùi cái gì, vì sao, ai quyết,
-- quyết lúc nào"; việc lùi vẫn do đúng service nghiệp vụ làm, vì chỉ nó biết phải dọn
-- những gì kèm theo.
CREATE TABLE reversal_requests (
    id           INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,

    -- Khoá trong sổ đăng ký của reversalService, vd 'expense.approve'. Cố ý để TEXT
    -- không CHECK: thêm một loại thao tác lùi là việc của mã nguồn, không nên phải chạy
    -- migration. Mã đã từ chối khoá lạ ngay ở cửa vào.
    kind         TEXT NOT NULL,
    entity_type  TEXT NOT NULL,
    entity_id    INT  NOT NULL,

    reason       TEXT NOT NULL CHECK (btrim(reason) <> ''),
    requested_by INT  NOT NULL REFERENCES profiles(id),
    requested_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),

    status       TEXT NOT NULL DEFAULT 'pending'
                     CHECK (status IN ('pending', 'approved', 'rejected', 'cancelled')),

    decided_by      INT REFERENCES profiles(id),
    decided_at      TIMESTAMPTZ,
    decision_note   TEXT,

    -- Tách khỏi status vì "đã duyệt" và "đã lùi xong" là hai việc: duyệt xong mà service
    -- nghiệp vụ từ chối (khoản đã chi mất rồi) thì yêu cầu vẫn approved nhưng không có gì
    -- được lùi — phải nhìn thấy được điều đó.
    executed_at     TIMESTAMPTZ,
    execution_error TEXT,

    CONSTRAINT reversal_requests_decided_together CHECK (
        (status = 'pending' AND decided_by IS NULL AND decided_at IS NULL)
        OR (status <> 'pending' AND decided_by IS NOT NULL AND decided_at IS NOT NULL)
    )
);

-- Một đối tượng chỉ được có MỘT yêu cầu đang chờ — không thì tài xế sốt ruột bấm năm
-- lần và điều phối thấy năm dòng giống hệt nhau.
CREATE UNIQUE INDEX uq_reversal_requests_pending
    ON reversal_requests (entity_type, entity_id)
    WHERE status = 'pending';

CREATE INDEX idx_reversal_requests_pending
    ON reversal_requests (status, requested_at)
    WHERE status = 'pending';

CREATE INDEX idx_reversal_requests_requester
    ON reversal_requests (requested_by, requested_at DESC);

CREATE TABLE shipment_assignment_history (
    id                  INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    shipment_id         INT NOT NULL REFERENCES order_shipments(id) ON DELETE CASCADE,

    from_driver_id      INT REFERENCES profiles(id),
    from_vehicle_id     INT REFERENCES vehicles(id),

    to_driver_id        INT REFERENCES profiles(id),
    to_vehicle_id       INT REFERENCES vehicles(id),

    changed_by          INT NOT NULL REFERENCES profiles(id),
    change_reason       TEXT NOT NULL
                            CHECK (change_reason IN (
                                'initial_assign',
                                'self_claim',
                                'incident_reassign',
                                'coordinator_swap',
                                'driver_request',
                                'release_to_pool',
                                'other'
                            )),
    incident_id         INT REFERENCES incidents(id),
    notes               TEXT,
    changed_at          TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE VIEW v_shipment_current AS
SELECT DISTINCT ON (h.shipment_id)
    h.shipment_id,
    h.to_driver_id   AS owner_driver_id,
    h.to_vehicle_id  AS vehicle_id
FROM shipment_assignment_history h
ORDER BY h.shipment_id, h.changed_at DESC, h.id DESC;

CREATE TABLE shipment_revenue_allocations (
    id                  INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    shipment_id         INT NOT NULL REFERENCES order_shipments(id) ON DELETE CASCADE,
    driver_id           INT NOT NULL REFERENCES profiles(id),
    share_percent       NUMERIC(5,2) NOT NULL CHECK (share_percent > 0 AND share_percent <= 100),
    allocation_reason   TEXT NOT NULL
                            CHECK (allocation_reason IN (
                                'default_owner',
                                'incident_full_transfer',
                                'incident_split'
                            )),
    incident_id         INT REFERENCES incidents(id) ON DELETE SET NULL,
    created_by          INT REFERENCES profiles(id),
    created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    UNIQUE (shipment_id, driver_id)
);

CREATE INDEX idx_assign_hist_shipment   ON shipment_assignment_history(shipment_id);
CREATE INDEX idx_assign_hist_from_drv   ON shipment_assignment_history(from_driver_id);
CREATE INDEX idx_assign_hist_to_drv     ON shipment_assignment_history(to_driver_id);
CREATE INDEX idx_assign_hist_incident   ON shipment_assignment_history(incident_id);
CREATE INDEX idx_rev_alloc_shipment     ON shipment_revenue_allocations(shipment_id);
CREATE INDEX idx_rev_alloc_driver       ON shipment_revenue_allocations(driver_id);

CREATE INDEX idx_accounts_email           ON accounts(email);
CREATE INDEX idx_profiles_role_id         ON profiles(role_id);

CREATE INDEX idx_vehicles_group_id        ON vehicles(vehicle_group_id);
CREATE INDEX idx_vehicles_status          ON vehicles(status);

CREATE INDEX idx_orders_customer_id       ON orders(customer_id);
CREATE INDEX idx_orders_derived_status    ON orders(derived_status);
CREATE INDEX idx_orders_created_by        ON orders(created_by);
CREATE INDEX idx_orders_created_at        ON orders(created_at DESC);

CREATE INDEX idx_shipments_order_id           ON order_shipments(order_id);
CREATE INDEX idx_shipments_status             ON order_shipments(status);
CREATE INDEX idx_shipments_pool               ON order_shipments(status)
    WHERE status = 'available';

CREATE INDEX idx_trip_stops_shipment_id       ON trip_stops(shipment_id);

CREATE INDEX idx_order_rr_order_id            ON order_receipt_requests(order_id);
CREATE INDEX idx_order_rr_driver_id           ON order_receipt_requests(driver_id);
CREATE INDEX idx_order_rr_status              ON order_receipt_requests(status)
    WHERE status IN ('pending','processing');

CREATE INDEX idx_sreceipts_shipment_id    ON shipment_receipts(shipment_id);
CREATE INDEX idx_sreceipts_orr_id         ON shipment_receipts(order_receipt_request_id);
CREATE INDEX idx_shipments_completed_at   ON order_shipments(completed_at) WHERE status = 'completed';
CREATE INDEX idx_dproofs_shipment_id      ON delivery_proofs(shipment_id);

CREATE INDEX idx_expenses_shipment_id     ON expenses(shipment_id);
CREATE INDEX idx_expenses_vehicle_id      ON expenses(vehicle_id);
CREATE INDEX idx_expenses_created_by      ON expenses(created_by);

CREATE INDEX idx_debts_customer_id        ON debts(customer_id);
CREATE INDEX idx_debts_driver_id          ON debts(driver_id);
CREATE INDEX idx_debts_shipment_id        ON debts(shipment_id);
CREATE INDEX idx_debts_order_id           ON debts(order_id);
CREATE INDEX idx_debt_payments_debt_status ON debt_payments(debt_id, status);
CREATE INDEX idx_debt_payments_pending     ON debt_payments(status) WHERE status = 'pending';

CREATE INDEX idx_invoices_customer_id     ON invoices(customer_id);
CREATE INDEX idx_invoices_order_id        ON invoices(order_id);
CREATE INDEX idx_invoices_status          ON invoices(status);
CREATE INDEX idx_inv_shipments_invoice    ON invoice_shipments(invoice_id);
CREATE INDEX idx_inv_shipments_shipment   ON invoice_shipments(shipment_id);

CREATE INDEX idx_kpi_driver_month_year    ON kpi_records(driver_id, year, month);
CREATE INDEX idx_dvgh_driver              ON driver_vehicle_group_history(driver_id, created_at DESC);
CREATE INDEX idx_kpi_group_month_year     ON kpi_records(vehicle_group_id, year, month);

CREATE INDEX idx_bonus_records_driver_id  ON bonus_records(driver_id);

-- Mỗi nhóm xe chỉ được có tối đa MỘT rule đang bật cho mỗi loại thưởng. Thiếu ràng buộc
-- này thì lookup (ORDER BY ... LIMIT 1) âm thầm chọn rule cũ hơn: quản lý sửa chính sách
-- bằng cách tạo rule mới sẽ thấy lương không đổi mà không hiểu vì sao.
CREATE UNIQUE INDEX uq_bonus_rules_active_group_type
    ON bonus_rules (vehicle_group_id, bonus_type)
    WHERE is_active;
-- NULL không đụng nhau trong UNIQUE nên rule áp dụng chung cần index riêng.
CREATE UNIQUE INDEX uq_bonus_rules_active_global_type
    ON bonus_rules (bonus_type)
    WHERE is_active AND vehicle_group_id IS NULL;

CREATE INDEX idx_payrolls_driver_id       ON payrolls(driver_id);
CREATE INDEX idx_payrolls_status          ON payrolls(status);
CREATE INDEX idx_payrolls_month_year      ON payrolls(payroll_year, payroll_month);

CREATE INDEX idx_advances_driver_id       ON salary_advances(driver_id);
CREATE INDEX idx_advances_status          ON salary_advances(status);

CREATE INDEX idx_incidents_shipment_id    ON incidents(shipment_id);
CREATE INDEX idx_incidents_vehicle_id     ON incidents(vehicle_id);
CREATE INDEX idx_incidents_status         ON incidents(status);

CREATE INDEX idx_notifications_user_id    ON notifications(user_id);
CREATE INDEX idx_notifications_unread     ON notifications(user_id) WHERE is_read = FALSE;

CREATE INDEX idx_logs_user_id             ON activity_logs(user_id);
CREATE INDEX idx_logs_entity              ON activity_logs(entity_type, entity_id);
CREATE INDEX idx_logs_created_at          ON activity_logs(created_at DESC);

CREATE INDEX idx_maintenance_vehicle_id   ON maintenance_records(vehicle_id);
CREATE INDEX idx_maintenance_vehicle_open ON maintenance_records(vehicle_id, status);
CREATE INDEX idx_maintenance_performed_by ON maintenance_records(performed_by);
CREATE INDEX idx_vehicle_status_hist_vh   ON vehicle_status_history(vehicle_id, created_at DESC);

CREATE VIEW v_leaderboard AS
SELECT
    k.vehicle_group_id,
    vg.name                     AS vehicle_group_name,
    k.year,
    k.month,
    k.driver_id,
    p.full_name                 AS driver_name,
    k.completed_shipments,
    k.total_revenue,
    k.incident_count,
    RANK() OVER (
        PARTITION BY k.vehicle_group_id, k.year, k.month
        ORDER BY k.total_revenue DESC
    )                           AS revenue_rank,
    RANK() OVER (
        PARTITION BY k.vehicle_group_id, k.year, k.month
        ORDER BY k.completed_shipments DESC
    )                           AS trips_rank
FROM kpi_records k
JOIN profiles p       ON p.id  = k.driver_id
JOIN vehicle_groups vg ON vg.id = k.vehicle_group_id;

CREATE VIEW v_trip_pool AS
SELECT
    os.id               AS shipment_id,
    os.order_id,
    os.vehicle_group_id,
    vg.name             AS vehicle_group_name,
    os.estimated_price,
    os.cargo_name,
    os.cargo_weight_kg,
    os.status,
    os.version,
    (SELECT address FROM trip_stops
     WHERE shipment_id = os.id AND stop_type = 'pickup'
     ORDER BY stop_index LIMIT 1)  AS first_pickup,
    (SELECT address FROM trip_stops
     WHERE shipment_id = os.id AND stop_type = 'delivery'
     ORDER BY stop_index DESC LIMIT 1) AS last_delivery,
    os.created_at
FROM order_shipments os
LEFT JOIN vehicle_groups vg ON vg.id = os.vehicle_group_id
WHERE os.status = 'available';

CREATE VIEW v_driver_debt_summary AS
SELECT
    d.driver_id,
    p.full_name                                                         AS driver_name,
    d.total_amount - COALESCE(SUM(dp.amount) FILTER (WHERE dp.status = 'confirmed'), 0) AS remaining_debt,
    COUNT(d.id)                                                         AS open_debt_count
FROM debts d
JOIN profiles p ON p.id = d.driver_id
LEFT JOIN debt_payments dp ON dp.debt_id = d.id
WHERE d.debt_type = 'driver'
GROUP BY d.id, d.driver_id, p.full_name, d.total_amount
HAVING d.total_amount - COALESCE(SUM(dp.amount) FILTER (WHERE dp.status = 'confirmed'), 0) > 0;

CREATE VIEW v_customer_debt_summary AS
SELECT
    c.id                                                                AS customer_id,
    COALESCE(c.company_name, c.full_name)                               AS customer_name,
    d.total_amount - COALESCE(SUM(dp.amount) FILTER (WHERE dp.status = 'confirmed'), 0) AS remaining_debt,
    COUNT(d.id) FILTER (WHERE d.due_date < CURRENT_DATE)                AS overdue_count
FROM debts d
JOIN customers c ON c.id = d.customer_id
LEFT JOIN debt_payments dp ON dp.debt_id = d.id
WHERE d.debt_type = 'customer'
GROUP BY d.id, c.id, c.company_name, c.full_name, d.total_amount, d.due_date
HAVING d.total_amount - COALESCE(SUM(dp.amount) FILTER (WHERE dp.status = 'confirmed'), 0) > 0;

CREATE TABLE financial_transactions (
    id              BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,

    event_type      TEXT NOT NULL CHECK (event_type IN (
                        'shipment_revenue',
                        'prepaid_received',
                        'prepaid_refunded',
                        'cash_receipt',
                        'bank_receipt',
                        'driver_debt_created',
                        'driver_debt_paid',
                        'customer_debt_created',
                        'customer_payment',
                        'pass_through_cost',
                        'expense_recorded',
                        -- Chi tiền hoàn ứng cho tài xế qua phiếu chi (Nợ 334 | Có 1111/1121).
                        -- Tách khỏi 'expense_recorded' vì chi phí đã được ghi nhận từ lúc
                        -- DUYỆT khoản chi; bước này chỉ tất toán khoản phải trả tài xế.
                        'expense_reimbursed',
                        'payroll_paid',
                        'bonus_paid',
                        'advance_disbursed',
                        'advance_recovered',
                        'debt_transferred',
                        -- Số dư đầu kỳ: nợ có TRƯỚC khi dùng phần mềm, kế toán khai tay.
                        -- Cần vế ghi Nợ này, nếu không thì lúc thu tiền sổ ghi Có 131 cho
                        -- khoản chưa từng ghi Nợ và tài khoản 131 sẽ âm.
                        'opening_balance',
                        -- Thu hộ (COD): công ty/tài xế đang giữ tiền hàng của khách.
                        -- Nợ <nơi tiền đang nằm> | Có 3388 — khoản PHẢI TRẢ LẠI khách,
                        -- không phải doanh thu, không phải tiền khách nợ. Tách tên riêng
                        -- khỏi sự kiện cha để getPassThroughOutstanding không đếm nhầm
                        -- vế thu hộ thành "đã thu lại tiền chi hộ".
                        'collect_on_behalf_held',
                        -- Đã trả tiền thu hộ lại cho người bán: Nợ 3388 | Có 1111/1121.
                        -- Cặp với dòng trên; thiếu nó thì 3388 chỉ phình mãi không đóng được.
                        'collect_on_behalf_returned'
                    )),

    debit_account   TEXT NOT NULL,
    credit_account  TEXT NOT NULL,
    amount          NUMERIC(12,2) NOT NULL CHECK (amount > 0 AND amount < 'Infinity'::numeric),
    description     TEXT,
    ref_type        TEXT CHECK (ref_type IN (
                        'shipment', 'order', 'debt',
                        'expense', 'payroll', 'advance', 'pass_through_cost', 'voucher'
                    )),
    ref_id          INT,
    actor_id        INT REFERENCES profiles(id),
    occurred_at     TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    exported_at     TIMESTAMPTZ,
    export_batch_id TEXT,
    reversal_of_id  BIGINT REFERENCES financial_transactions(id),
    reversal_reason TEXT
);

CREATE INDEX idx_ftx_event_type    ON financial_transactions(event_type);
CREATE INDEX idx_ftx_ref           ON financial_transactions(ref_type, ref_id);
CREATE INDEX idx_ftx_actor         ON financial_transactions(actor_id);
CREATE INDEX idx_ftx_occurred_at   ON financial_transactions(occurred_at DESC);
CREATE INDEX idx_ftx_export        ON financial_transactions(exported_at) WHERE exported_at IS NULL;
CREATE INDEX idx_ftx_reversal_of   ON financial_transactions(reversal_of_id) WHERE reversal_of_id IS NOT NULL;
-- Tra "chi hộ của đơn này đã thu được bao nhiêu" chạy trên MỌI lần tiền khách về
CREATE INDEX idx_ftx_pass_through_settlement
    ON financial_transactions(ref_type, ref_id)
    WHERE credit_account = '3388' OR debit_account = '3388';

CREATE TABLE payment_vouchers (
    id               INT GENERATED BY DEFAULT AS IDENTITY (START WITH 100000 INCREMENT BY 1) PRIMARY KEY,
    voucher_type     TEXT NOT NULL CHECK (voucher_type IN ('office','rent','utilities','equipment','entertainment','compensation','prepaid_refund','driver_reimbursement','collect_on_behalf_return','other')),
    amount           NUMERIC(12,2) NOT NULL CHECK (amount > 0 AND amount < 'Infinity'::numeric),
    payee            TEXT NOT NULL,
    reason           TEXT NOT NULL,
    payment_method   TEXT NOT NULL DEFAULT 'cash' CHECK (payment_method IN ('cash','bank_transfer')),
    proof_url        TEXT,
    status           TEXT NOT NULL DEFAULT 'pending' CHECK (status IN ('pending','approved','rejected','paid','cancelled')),
    rejection_reason TEXT,
    cancelled_by        INT REFERENCES profiles(id),
    cancelled_at        TIMESTAMPTZ,
    cancellation_reason TEXT,
    incident_id      INT REFERENCES incidents(id) ON DELETE SET NULL,
    order_id         INT REFERENCES orders(id) ON DELETE SET NULL,
    -- Khoản chi phí tài xế đã ứng mà phiếu này hoàn lại (voucher_type = driver_reimbursement).
    -- Thiếu liên kết này thì chi tiền xong expenses.reimbursement_status vẫn 'pending' và
    -- kỳ lương sẽ hoàn LẦN HAI.
    expense_id       INT REFERENCES expenses(id) ON DELETE SET NULL,
    created_by       INT NOT NULL REFERENCES profiles(id),
    approved_by      INT REFERENCES profiles(id),
    paid_by          INT REFERENCES profiles(id),
    approved_at      TIMESTAMPTZ,
    paid_at          TIMESTAMPTZ,
    created_at       TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at       TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE INDEX idx_payment_vouchers_status     ON payment_vouchers(status);
CREATE INDEX idx_payment_vouchers_created_at ON payment_vouchers(created_at DESC);
CREATE INDEX idx_payment_vouchers_incident   ON payment_vouchers(incident_id) WHERE incident_id IS NOT NULL;
-- Mỗi khoản chi phí chỉ được có MỘT phiếu hoàn ứng còn hiệu lực. Phiếu bị từ chối/huỷ
-- không tính, để kế toán lập lại được.
CREATE UNIQUE INDEX uniq_live_reimbursement_voucher_per_expense
    ON payment_vouchers(expense_id)
    WHERE expense_id IS NOT NULL AND status IN ('pending', 'approved', 'paid');
CREATE INDEX idx_payment_vouchers_order      ON payment_vouchers(order_id) WHERE order_id IS NOT NULL;

CREATE TABLE driver_bonuses (
    id                      SERIAL PRIMARY KEY,
    driver_id               INT            NOT NULL REFERENCES profiles(id) ON DELETE CASCADE,
    type                    VARCHAR(30)    NOT NULL CHECK (type IN (
                                'tet_annual', 'welfare_wedding', 'welfare_funeral',
                                'welfare_birthday', 'holiday_overtime', 'special'
                            )),
    year                    SMALLINT       NOT NULL,

    amount                  NUMERIC(12,2)  NOT NULL DEFAULT 0 CHECK (amount >= 0 AND amount < 'Infinity'::numeric),
    notes                   TEXT,

    months_full_count       SMALLINT,
    months_incomplete_count SMALLINT,
    seniority_bonus         NUMERIC(12,2),
    attendance_bonus        NUMERIC(12,2),

    beneficiary_name        VARCHAR(200),
    beneficiary_relation    VARCHAR(30)    CHECK (beneficiary_relation IN (
                                'self', 'spouse', 'parent', 'parent_in_law', 'child'
                            )),
    proof_url               TEXT,

    status                  VARCHAR(20)    NOT NULL DEFAULT 'pending' CHECK (status IN (
                                'pending', 'approved', 'rejected', 'paid'
                            )),
    requested_by            INT            REFERENCES profiles(id),
    approved_by             INT            REFERENCES profiles(id),
    paid_by                 INT            REFERENCES profiles(id),
    rejection_reason        TEXT,

    requested_at            TIMESTAMPTZ    DEFAULT NOW(),
    approved_at             TIMESTAMPTZ,
    paid_at                 TIMESTAMPTZ,
    created_at              TIMESTAMPTZ    DEFAULT NOW(),
    updated_at              TIMESTAMPTZ    DEFAULT NOW()
);

CREATE UNIQUE INDEX uq_driver_tet_year
    ON driver_bonuses (driver_id, year)
    WHERE type = 'tet_annual';

CREATE INDEX idx_driver_bonuses_driver ON driver_bonuses (driver_id);
CREATE INDEX idx_driver_bonuses_status ON driver_bonuses (status);
CREATE INDEX idx_driver_bonuses_type   ON driver_bonuses (type);
CREATE INDEX idx_driver_bonuses_year   ON driver_bonuses (year);

CREATE INDEX idx_expenses_status ON expenses(status);
CREATE INDEX idx_maintenance_requested
    ON maintenance_records(status) WHERE status = 'requested';
CREATE INDEX idx_vda_vehicle ON vehicle_driver_assignments(vehicle_id);
CREATE INDEX idx_vda_driver  ON vehicle_driver_assignments(driver_id);
CREATE INDEX idx_vda_prev_driver ON vehicle_driver_assignments(previous_driver_id);

CREATE INDEX idx_shipments_vehicle_group  ON order_shipments(vehicle_group_id);
CREATE INDEX idx_debts_due_date           ON debts(due_date);
CREATE INDEX idx_invoices_due_date        ON invoices(due_date);
CREATE INDEX idx_invoices_invoice_date    ON invoices(invoice_date);

CREATE TABLE device_tokens (
    id          SERIAL PRIMARY KEY,
    user_id     INT NOT NULL REFERENCES profiles(id) ON DELETE CASCADE,
    token       TEXT NOT NULL,
    platform    VARCHAR(10) NOT NULL DEFAULT 'android' CHECK (platform IN ('android', 'ios', 'web')),
    created_at  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    UNIQUE (token)
);

CREATE INDEX idx_device_tokens_user ON device_tokens(user_id);

-- Đối chiếu kết quả giao push của Expo.
--
-- Expo giao hàng theo HAI bước: POST /push/send trả "ticket" (Expo đã NHẬN message,
-- CHƯA gửi đi), POST /push/getReceipts mới trả "receipt" (FCM/APNs đã nhận chưa,
-- hỏng thì vì sao). Ticket "ok" KHÔNG có nghĩa là máy đã nhận.
--
-- Bảng này giữ ticket lại để cron/pushCron.js đối chiếu receipt sau vài phút: ghi log
-- mã lỗi thật và dọn token chết. Không có nó thì thiết bị bị Doze, credential FCM sai,
-- MessageRateExceeded... đều im lặng, không có bằng chứng nào để lần.
--
-- Lưu ở DB chứ không giữ trong RAM vì Cloud Run scale về 0 và restart bất kỳ lúc nào.
CREATE TABLE push_tickets (
    id          BIGSERIAL PRIMARY KEY,
    ticket_id   TEXT        NOT NULL,
    token       TEXT        NOT NULL,   -- giữ lại để dọn khi receipt báo DeviceNotRegistered
    user_id     INT         REFERENCES profiles(id) ON DELETE SET NULL,
    sent_at     TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    checked_at  TIMESTAMPTZ,            -- NULL = chưa đối chiếu receipt
    status      VARCHAR(20),            -- 'ok' | 'error'
    error_code  TEXT,
    UNIQUE (ticket_id)
);

-- Index một phần: cron chỉ quét đúng các ticket chưa đối chiếu, không đụng lịch sử.
CREATE INDEX idx_push_tickets_pending ON push_tickets(sent_at) WHERE checked_at IS NULL;
CREATE INDEX idx_debt_payments_paid_at    ON debt_payments(paid_at);

-- Kỳ báo cáo kinh doanh đã KÝ DUYỆT (khoá cứng, snapshot đóng băng).
-- Chỉ đúng hai trạng thái: không có dòng = kỳ đang mở, tính động thời gian thực;
-- có dòng = đã ký duyệt, số không đổi nữa. Không có bước "chốt" trung gian.
CREATE TABLE business_report_periods (
    id              INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    period_year     SMALLINT NOT NULL,
    period_month    SMALLINT NOT NULL CHECK (period_month BETWEEN 1 AND 12),
    status          TEXT NOT NULL DEFAULT 'signed_off'
                        CHECK (status = 'signed_off'),
    snapshot        JSONB NOT NULL,

    revenue         NUMERIC(14,2) NOT NULL DEFAULT 0,
    operating_cost  NUMERIC(14,2) NOT NULL DEFAULT 0,
    payroll_cost    NUMERIC(14,2) NOT NULL DEFAULT 0,
    gross_profit    NUMERIC(14,2) NOT NULL DEFAULT 0,
    margin_pct      NUMERIC(6,2)  NOT NULL DEFAULT 0,

    note            TEXT,
    signed_off_by   INT NOT NULL REFERENCES profiles(id),
    signed_off_at   TIMESTAMPTZ NOT NULL DEFAULT NOW(),

    created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),

    UNIQUE (period_year, period_month)
);

CREATE INDEX idx_business_report_periods_ym
    ON business_report_periods(period_year, period_month);

-- ── Kỳ ghi nhận doanh thu ───────────────────────────────────────────────────
-- Phải đặt SAU business_report_periods: hàm bên dưới đọc bảng đó và Postgres kiểm
-- tra thân hàm SQL ngay lúc CREATE.

-- Kỳ ghi nhận cho một chuyến hoàn thành lúc p_completed_at: tháng của chính nó nếu
-- kỳ đó còn mở, ngược lại là kỳ MỞ SỚM NHẤT kế tiếp. business_report_periods chỉ
-- chứa kỳ đã ký duyệt nên "có dòng" = "đã khoá". generate_series + MIN xử lý đúng cả
-- chuỗi nhiều tháng đã ký liên tiếp lẫn trường hợp thủng ở giữa. Trần 120 tháng chỉ
-- để chặn vòng lặp vô hạn nếu dữ liệu bẩn.
CREATE OR REPLACE FUNCTION fn_revenue_recognition_period(p_completed_at TIMESTAMPTZ)
RETURNS DATE
LANGUAGE sql
STABLE
AS $$
    WITH base AS (
        SELECT date_trunc('month', p_completed_at AT TIME ZONE 'Asia/Ho_Chi_Minh')::date AS p
    )
    SELECT COALESCE(
        (
            SELECT MIN(g.p)::date
            FROM base,
                 generate_series(base.p, base.p + INTERVAL '120 months', INTERVAL '1 month') g(p)
            WHERE NOT EXISTS (
                SELECT 1 FROM business_report_periods brp
                WHERE make_date(brp.period_year, brp.period_month, 1) = g.p::date
            )
        ),
        (SELECT p FROM base)
    );
$$;

CREATE OR REPLACE FUNCTION trg_set_revenue_period() RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
    v_recompute BOOLEAN := FALSE;
BEGIN
    IF NEW.status = 'completed' AND NEW.completed_at IS NOT NULL THEN
        IF TG_OP = 'INSERT' THEN
            v_recompute := TRUE;
        ELSE
            v_recompute := OLD.status       IS DISTINCT FROM 'completed'
                        OR OLD.completed_at IS DISTINCT FROM NEW.completed_at
                        OR (OLD.actual_price IS NULL AND NEW.actual_price IS NOT NULL);
        END IF;
    END IF;

    IF v_recompute THEN
        NEW.revenue_period := fn_revenue_recognition_period(NEW.completed_at);
    END IF;
    RETURN NEW;
END;
$$;

CREATE TRIGGER set_revenue_period
    BEFORE INSERT OR UPDATE ON order_shipments
    FOR EACH ROW EXECUTE FUNCTION trg_set_revenue_period();

CREATE INDEX idx_order_shipments_revenue_period
    ON order_shipments (revenue_period)
    WHERE status = 'completed';

-- ══ DỮ LIỆU CỐ ĐỊNH ══════════════════════════════════════════════════════
-- Tài khoản, nhóm xe, xe, tài xế và CHẾ ĐỘ THƯỞNG. Đây là danh mục nền, luôn
-- tồn tại từ đầu và reset_data.sql KHÔNG xoá. Sinh bởi tools/gen_seed.py.

INSERT INTO accounts (id, email, password_hash, role_id, is_active) VALUES
    (100000, 'anhdv76@gmail.com', crypt('Admin@1234',  gen_salt('bf')), (SELECT id FROM roles WHERE name = 'manager'),     TRUE),
    (100001, 'cuongnt@gmail.com', crypt('Coord@1234',  gen_salt('bf')), (SELECT id FROM roles WHERE name = 'coordinator'), TRUE),
    (100002, 'hoangtm@gmail.com', crypt('Acct@1234',   gen_salt('bf')), (SELECT id FROM roles WHERE name = 'accountant'),  TRUE),
    (100003, 'tienpv@gmail.com',  crypt('Driver@1234', gen_salt('bf')), (SELECT id FROM roles WHERE name = 'driver'),      TRUE),
    (100004, 'quanhm@gmail.com',  crypt('Driver@1234', gen_salt('bf')), (SELECT id FROM roles WHERE name = 'driver'),      TRUE),
    (100005, 'phuocdh@gmail.com', crypt('Driver@1234', gen_salt('bf')), (SELECT id FROM roles WHERE name = 'driver'),      TRUE),
    (100006, 'sonlt@gmail.com',   crypt('Driver@1234', gen_salt('bf')), (SELECT id FROM roles WHERE name = 'driver'),      TRUE);

INSERT INTO profiles (id, full_name, phone, role_id, dob, gender, national_id, address, city, country, emergency_contact_name, emergency_contact_phone, notes) VALUES
    (100000, 'Đỗ Việt Anh',       '0901000001', (SELECT id FROM roles WHERE name = 'manager'),     '1985-04-12', 'male',   '079185000111', '12 Nguyễn Huệ, Quận 1',      'Hồ Chí Minh', 'VN', 'Nguyễn Thị Hoa', '0908000001', 'Giám đốc điều hành'),
    (100001, 'Nguyễn Thế Cương',  '0901000002', (SELECT id FROM roles WHERE name = 'coordinator'), '1992-09-21', 'male',   '079192000222', '88 Lê Lợi, Quận 3',          'Hồ Chí Minh', 'VN', 'Nguyễn Văn Phúc','0908000002', 'Điều phối viên'),
    (100002, 'Triệu Minh Hoàng',  '0901000003', (SELECT id FROM roles WHERE name = 'accountant'),  '1991-03-14', 'male',   '079191000333', '25 Võ Thị Sáu, Quận 3',      'Hồ Chí Minh', 'VN', 'Triệu Minh Châu','0908000003', 'Kế toán tổng hợp'),
    (100003, 'Phạm Văn Tiền',     '0901000004', (SELECT id FROM roles WHERE name = 'driver'),      '1993-07-08', 'male',   '079193000444', '101 Trần Hưng Đạo, Thủ Đức', 'Hồ Chí Minh', 'VN', 'Phạm Thị Lan',   '0908000004', 'Tài xế xe cắt nóc'),
    (100004, 'Hoàng Minh Quân',   '0901000005', (SELECT id FROM roles WHERE name = 'driver'),      '1990-11-02', 'male',   '079190000555', '55 Hùng Vương, Quận 5',      'Hồ Chí Minh', 'VN', 'Hoàng Thị Hạnh', '0908000005', 'Tài xế xe 3 tấn'),
    (100005, 'Đỗ Hữu Phước',      '0901000006', (SELECT id FROM roles WHERE name = 'driver'),      '1996-01-18', 'male',   '079196000666', '7 Nguyễn Văn Cừ, Quận 5',    'Hồ Chí Minh', 'VN', 'Đỗ Quốc Minh',   '0908000006', 'Tài xế xe 5m2'),
    (100006, 'Lê Thanh Sơn',      '0901000007', (SELECT id FROM roles WHERE name = 'driver'),      '1995-05-30', 'male',   '079195000777', '210 Phan Văn Trị, Gò Vấp',   'Hồ Chí Minh', 'VN', 'Lê Thị Mai',     '0908000007', 'Tài xế xe cắt nóc (xe thứ 2)');

INSERT INTO vehicle_groups (id, name, description, max_load_weight_kg, price_per_km) VALUES
    (100000, 'Xe cắt nóc', 'Xe tải nhẹ cắt nóc, chở hàng cồng kềnh nội thành', 2000, 10000),
    (100001, 'Xe 3 tấn (4m3)', 'Xe tải 3 tấn thùng 4m3', 3000, 15000),
    (100002, 'Xe 5m2', 'Xe tải thùng dài 5m2', 7000, 20000),
    (100003, 'Xe 7m4', 'Xe tải thùng dài 7m4, tuyến liên tỉnh', 10000, 30000);

INSERT INTO vehicles (id, plate_number, vehicle_group_id, brand, model, load_capacity_kg, manufacture_year, purchase_date, assigned_driver_id, status) VALUES
    (100000, '51C-123.45', 100000, 'Suzuki', 'Carry Pro', 1900, 2022, DATE '2022-06-10', 100003, 'active'),
    (100001, '51D-678.90', 100001, 'Isuzu', 'QKR 230', 3000, 2021, DATE '2021-09-15', 100004, 'active'),
    (100002, '51E-246.80', 100002, 'Hino', 'XZU 342L', 6800, 2023, DATE '2023-02-20', 100005, 'active'),
    (100003, '51F-135.79', 100003, 'Hyundai', 'Mighty EX8', 9500, 2022, DATE '2022-11-05', NULL, 'active'),
    (100004, '51H-889.12', 100000, 'Thaco', 'Towner 990', 1900, 2023, DATE '2023-08-12', 100006, 'active');

INSERT INTO drivers (profile_id, vehicle_id, default_vehicle_group_id, license_number, license_expiry_date, hire_date, revenue_share_percent, emergency_contact_name, emergency_contact_phone) VALUES
    (100003, 100000, 100000, 'DL-0123456', DATE '2029-12-31', DATE '2023-03-01', 15.0, 'Người thân Tiền', '0908000004'),
    (100004, 100001, 100001, 'DL-0234567', DATE '2029-12-31', DATE '2024-05-15', 15.0, 'Người thân Quân', '0908000005'),
    (100005, 100002, 100002, 'DL-0345678', DATE '2029-12-31', DATE '2025-11-01', 15.0, 'Người thân Phước', '0908000006'),
    (100006, 100004, 100000, 'DL-0456789', DATE '2029-12-31', DATE '2025-09-20', 15.0, 'Người thân Sơn', '0908000007');

-- Chế độ thưởng theo "Thông báo thay đổi chính sách tiền lương" 29/03/2026,
-- hiệu lực 01/04/2026. KHÔNG sửa các con số này khi làm dữ liệu demo.
--   Điều II.4 — Thưởng cuối tháng: 3 giải "Lái xe xuất sắc nhất tháng",
--               1.000.000đ/giải, chỉ cho 3 nhóm: cắt nóc, 3 tấn (4m3), 5m2.
--   Điều II.5 — Thưởng vượt KPI 2.000.000đ theo ngưỡng doanh thu/tháng:
--               cắt nóc > 50tr, 4m3 > 65tr, 5m2 > 70tr, 7m4 > 100tr.
INSERT INTO bonus_rules (vehicle_group_id, title, bonus_type, reward_amount, conditions_json) VALUES
    (100000, 'Thưởng vượt KPI — Xe cắt nóc',                'kpi',         2000000, '{"min_revenue": 50000000}'::jsonb),
    (100001, 'Thưởng vượt KPI — Xe 3 tấn (4m3)',            'kpi',         2000000, '{"min_revenue": 65000000}'::jsonb),
    (100002, 'Thưởng vượt KPI — Xe 5m2',                    'kpi',         2000000, '{"min_revenue": 70000000}'::jsonb),
    (100003, 'Thưởng vượt KPI — Xe 7m4',                    'kpi',         2000000, '{"min_revenue": 100000000}'::jsonb),
    (100000, 'Lái xe xuất sắc nhất tháng — Xe cắt nóc',     'top_revenue', 1000000, '{"rank": 1}'::jsonb),
    (100001, 'Lái xe xuất sắc nhất tháng — Xe 3 tấn (4m3)', 'top_revenue', 1000000, '{"rank": 1}'::jsonb),
    (100002, 'Lái xe xuất sắc nhất tháng — Xe 5m2',         'top_revenue', 1000000, '{"rank": 1}'::jsonb);

--   Điều V.1 — đi làm ngày lễ hưởng 200% lương ngày. Áp dụng cho MỌI nhóm xe nên
--               vehicle_group_id để NULL. Con số 200% trước đây nằm cứng trong mã tính
--               lương; giờ là reward_multiplier = 2.00 ở đây — đổi chính sách chỉ cần sửa
--               dòng này (hoặc sửa qua màn Quy tắc thưởng), không phải deploy lại backend.
--               Lương cứng đã bao gồm 100% (ngày lễ không trừ công), nên bộ tính lương chỉ
--               cộng thêm phần vượt: (hệ số - 1) × lương ngày × số ngày lễ có đi làm.
INSERT INTO bonus_rules (vehicle_group_id, title, bonus_type, reward_multiplier) VALUES
    (NULL, 'Đi làm ngày lễ — hệ số lương (Điều V.1)', 'holiday', 2.00);

-- Danh mục trên được insert KÈM id tường minh (100000...), việc đó KHÔNG làm
-- identity sequence tự tăng. Không set lại thì lần đầu app tạo tài khoản / nhóm xe /
-- xe mới sẽ xin id = 100000 và lỗi duplicate key. Bắt buộc phải có, kể cả khi
-- deploy production không nạp seed.sql.
SELECT setval(pg_get_serial_sequence('accounts','id'),       (SELECT MAX(id) FROM accounts));
SELECT setval(pg_get_serial_sequence('vehicle_groups','id'), (SELECT MAX(id) FROM vehicle_groups));
SELECT setval(pg_get_serial_sequence('vehicles','id'),       (SELECT MAX(id) FROM vehicles));
